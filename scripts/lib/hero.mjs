...
export async function generateHeroAssets({ config, sourcePathDark, sourcePathLight, outputDirectory }) {
  const sourceBufferDark = await readFile(sourcePathDark);
  const sourceBufferLight = await readFile(sourcePathLight);
  await validatePortrait(sourceBufferDark, sourcePathDark);
  await validatePortrait(sourceBufferLight, sourcePathLight);

  const version = createHash("sha256")
    .update(GENERATOR_VERSION)
    .update(JSON.stringify(config))
    .update(sourceBufferDark)
    .update(sourceBufferLight)
    .digest("hex")
    .slice(0, 8);
  const palette = paletteDefinitions[config.appearance.palette];
  const desktopPortraitDark = await samplePortrait(sourceBufferDark, layouts.desktop.portrait.columns, layouts.desktop.portrait.rows);
  const desktopPortraitLight = await samplePortrait(sourceBufferLight, layouts.desktop.portrait.columns, layouts.desktop.portrait.rows);
  const mobilePortraitDark = await samplePortrait(sourceBufferDark, layouts.mobile.portrait.columns, layouts.mobile.portrait.rows);
  const mobilePortraitLight = await samplePortrait(sourceBufferLight, layouts.mobile.portrait.columns, layouts.mobile.portrait.rows);
  const assets = {
  ...
  await Promise.all([
    writeFile(resolve(outputDirectory, assets.desktopDark), createHeroSvg(config, palette.dark, "desktop", desktopPortraitDark)),
    writeFile(resolve(outputDirectory, assets.desktopLight), createHeroSvg(config, palette.light, "desktop", desktopPortraitLight)),
    writeFile(resolve(outputDirectory, assets.mobileDark), createHeroSvg(config, palette.dark, "mobile", mobilePortraitDark)),
    writeFile(resolve(outputDirectory, assets.mobileLight), createHeroSvg(config, palette.light, "mobile", mobilePortraitLight))
  ]);
